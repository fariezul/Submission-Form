...
create table if not exists public.quiz_attempts (
  ...
  -- 1500 s is the real limit; the extra headroom absorbs a slow
...
--
-- September 2026: the time limit is now 25 minutes. Each time it
-- rises, this cap has to rise with it: a full attempt writes
-- duration_seconds equal to the limit exactly, so the cap must
-- sit ABOVE it or the slowest legitimate attempts are rejected.
-- Limit 1500 s, cap 1800 s.
