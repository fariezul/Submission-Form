...
(function (root) {
  ...
  /* ----------------------------------------------------------
     SETTINGS
     ---------------------------------------------------------- */
  /* Activity 3 asks 30. This one asks 20, on the lecturer's
     instruction — the Lean section is a smaller body of material
     and does not need the same length of paper to examine.

     It has a second, useful effect. Drawing 20 from a bank of 50
     means about 8 of your 20 repeat on the next attempt, against
     19 of 30 at the old setting. Fewer questions per attempt
     makes each attempt LESS like the last one, not more. */
  const QUESTIONS_PER_ATTEMPT = 20;

  /* The whole attempt must fit inside this. Activity 3 started at
     5 minutes, went to 10 when students were not reaching the end,
     then to 25 for a 30-question paper. Activity 4 is 20 minutes
     for 20 questions — a minute each, which is the same pace the
     25/30 setting settled on.

     If you change it, THREE things must move with it, and
     quiz-tests.js fails if any is missed:
       - the wording on activity-4.html (the rule tile, the
         briefing warning, the starting value of the timer)
       - the duration clamp in activity-4-Code.gs, which needs
         headroom above the new limit (it is 3600 s, so there is
         room to grow without touching it)
       - the clock tests in quiz-tests.js

     Note what is NOT on that list any more. Activity 3 also had
     a duration_seconds constraint living on the live database,
     which twice stayed behind when the file changed and quietly
     rejected the longest attempts. There is no such second copy
     here: the Apps Script IS the deployed code. */
  const TIME_LIMIT_MS = 20 * 60 * 1000;   // 1 200 000 ms = 20:00

  /* How many of each type we try to include in every attempt.
     ----------------------------------------------------------
     Without this, drawing 20 at random from a bank that is
     ...
     bank, so the attempt is always exactly 20 questions.

     These came down with the paper: 8 typed questions out of 20
     rather than 9 out of 30. Left alone at the old numbers, a
     third of a shorter paper would have been fixed in advance,
     which is variety working against itself.

     IMAGE-CHOICE IS 1 HERE, NOT ACTIVITY 3's 3. Slides 36-56
     contain exactly one picture carrying a testable idea (the
     value/waste arrows on slide 37), so there is one image
     question to draw. Asking for 3 would not break anything —
     the top-up handles it — but it would be a promise the bank
     cannot keep, and quiz-tests.js checks quotas against real
     pool sizes for exactly that reason.
     ---------------------------------------------------------- */
  const TYPE_QUOTAS = {
    "image-choice": 1,
    "true-false": 3,
  ...
  function scoreAttempt(responses, totalQuestions) {
    ...
    let counted = 0;
    for (const r of responses) if (r.correct) counted++;

    /* Clamped to the paper size. In normal play it can never be
       reached — the responses are built from the questions on the
       paper — but nothing in the function guaranteed it, and an
       unclamped score gives a percentage over 100 and a score
       like "25 / 20" on the class scoreboard.

       Note that clamping does NOT create a false pass: completed
       still requires the score to equal the total exactly, and a
       run with more correct answers than questions is not a real
       run in the first place. */
    const score = Math.min(counted, total);
